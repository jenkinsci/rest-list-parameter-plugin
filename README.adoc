...
Once the values arrive, the parameter shows them with the usual dropdown, free-text or multi-select behaviour and default preselection.
...
The selected entry stays selected if it is still offered, typed free text is kept, and free-form entries of a multi-select are kept.
...
With it unchecked, the user can also type entries that are not in the list; each one is submitted verbatim as its own element, and the endpoint is not contacted to validate the submission.
This helps with paginated APIs whose wanted value is not on the first page.
...
Default values that match no entry are ignored when validation is enabled; when it is disabled, they are preselected as free-form entries.
