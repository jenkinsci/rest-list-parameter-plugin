...
All pages are requested one after another while the `Build with Parameters` form renders, so each page adds to the time the form takes to open.
Each page is cached individually for the `Cache Max Age`.
...
With `Enable Value Validation` checked (the default), every element must be the value of an entry fetched when the build is submitted.
If fetching fails at that time, any non-empty list is rejected.
